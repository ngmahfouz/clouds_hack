...
# Example of images generated by our GAN

A sample of generated images can be found in `img_sample.png`. The leftmost column represent the original image in the validation set. The rightmost column corresponds to the mean (average of pixels) image in the batch of the original image. The 10 columns in between are samples (with different noise) from our GAN using the conditioning meteorological variables of the original iamge (leftmost column)
